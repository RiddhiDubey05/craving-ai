...
export const semanticImageMap = [
  ...
  { keywords: ['paneer tikka', 'chicken tikka', 'tandoori chicken', 'tikka masala'], url: 'https://images.unsplash.com/photo-1599487488170-d11ec9c172f0?w=800&q=80' },
  { keywords: ['veg manchurian', 'gobi manchurian', 'manchurian'], url: 'https://images.unsplash.com/photo-1603133872878-684f208fb84b?w=800&q=80' },
  { keywords: ['veg biryani', 'chicken biryani', 'mutton biryani', 'biryani', 'pulao', 'mandi'], url: 'https://images.unsplash.com/photo-1631515243349-e0cb75fb8d3a?w=800&q=80' },
  { keywords: ['chocolate milkshake', 'cold coffee', 'milkshake', 'smoothie'], url: 'https://images.unsplash.com/photo-1497935586351-b67a49e012bf?w=800&q=80' },
  ...
  { keywords: ['fried rice', 'chilli'], url: 'https://images.unsplash.com/photo-1603133872878-684f208fb84b?w=800&q=80' },
  ...
  { keywords: ['salad', 'healthy', 'vegan', 'buddha bowl'], url: 'https://images.unsplash.com/photo-1512621776951-a57141f2eefd?w=800&q=80' },
  { keywords: ['egg', 'omelette', 'breakfast', 'waffle'], url: 'https://images.unsplash.com/photo-1533089860892-a7c6f0a88666?w=800&q=80' },
  { keywords: ['coffee', 'tea', 'latte', 'espresso', 'shake', 'juice', 'drink', 'beverage', 'mocktail'], url: 'https://images.unsplash.com/photo-1497935586351-b67a49e012bf?w=800&q=80' },
...
export const searchDishes = async (query, isVegOnly, city, country) => {
  ...
  try {
    const res = await fetch(API_BASE + '/api/search', {
      ...
      body: JSON.stringify({ query, isVegOnly, city: effectiveCity })
    });
    ...
    return [{
      ...
      type: isVegOnly ? 'veg' : 'non-veg',
      img: getImageUrl(safeQuery),
      zomato: formatPrice(basePrice, effectiveCountry),
      swiggy: formatPrice(basePrice - 15, effectiveCountry),
      toing: formatPrice(basePrice - 40, effectiveCountry),
      trending: true
    }];
  }
};

export const getTrendingDishes = async (isVegOnly, city, country) => {
  const effectiveCity = city || 'Mumbai';
  const effectiveCountry = country || 'India';
  try {
    const res = await fetch(API_BASE + '/api/trending', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ isVegOnly, city: effectiveCity })
    });
    ...
    const results = backendData.map(item => ({
      ...
      imageUrl: item.imageUrl || getImageUrl(item.dishName)
    }));
    ...
    return [
      { id: 901, dishName: 'Spicy Ramen', restaurant: 'Noodle Hub', price: formatPrice(450, effectiveCountry), rating: 4.8, platforms: ['Zomato', 'Swiggy'], imageUrl: getImageUrl('Spicy Ramen') },
      { id: 902, dishName: 'Truffle Pizza', restaurant: 'Oven Story', price: formatPrice(650, effectiveCountry), rating: 4.7, platforms: ['Zomato'], imageUrl: getImageUrl('Truffle Pizza') },
      { id: 903, dishName: 'Sushi Platter', restaurant: 'Zen Bento', price: formatPrice(1200, effectiveCountry), rating: 4.9, platforms: ['Swiggy'], imageUrl: getImageUrl('Sushi Platter') },
      { id: 904, dishName: 'Loaded Burger', restaurant: 'Burger Co', price: formatPrice(350, effectiveCountry), rating: 4.6, platforms: ['Zomato', 'Swiggy'], imageUrl: getImageUrl('Loaded Burger') }
    ];
  }
};
